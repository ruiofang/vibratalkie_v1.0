...
Esp32Camera::Esp32Camera(const esp_video_init_config_t& config) {
    ...
    // 某些版本/启动时序下，设备节点创建会有延迟：循环等待并尝试期望节点+回退节点
    int open_errno = 0;
    for (int retry = 0; retry < 50 && video_fd_ < 0; ++retry) {  // 最多约1.5s
        int fd = open(video_device_name, O_RDWR);
        if (fd >= 0) {
            video_fd_ = fd;
            break;
        }
        ...
        for (int i = 0; i <= 4 && video_fd_ < 0; ++i) {
            char path[16];
            snprintf(path, sizeof(path), "/dev/video%d", i);
            fd = open(path, O_RDWR);
            if (fd >= 0) {
                ESP_LOGW(TAG, "fallback opened %s (expected %s)", path, video_device_name);
                video_fd_ = fd;
                break;
            }
            open_errno = errno;
    ...
    if (video_fd_ < 0) {
        ESP_LOGE(TAG, "open %s failed finally, errno=%d(%s)", video_device_name, open_errno, strerror(open_errno));
#if CONFIG_XIAOZHI_ENABLE_CAMERA_DEBUG_MODE
