...
void PowerSaveTimer::PowerSaveCheck() {
    ...
    if (seconds_to_sleep_ != -1 && ticks_ >= seconds_to_sleep_) {
        if (!in_sleep_mode_) {
            ...
            if (cpu_max_freq_ != -1) {
                /*
                // 注意：为了允许休眠状态下通过语音唤醒设备，保留唤醒词检测和音频输入运行
                // 原实现会在休眠时关闭唤醒词检测和音频输入，导致无法通过语音唤醒。
                // 这里不再关闭它们，仍然降低CPU频率并启用light sleep以节能。
...
void PowerSaveTimer::SetTimeout(int seconds_to_sleep, int seconds_to_shutdown) {
    seconds_to_sleep_ = seconds_to_sleep;
    seconds_to_shutdown_ = seconds_to_shutdown;
    
    if (enabled_) {
        ticks_ = 0;
        if (in_sleep_mode_ && seconds_to_sleep_ == -1) {
            WakeUp();
        }
    }
    //LOG_INFO("Power save: enabled=%d, sleep=%ds, shutdown=%ds", enabled_, seconds_to_sleep_, seconds_to_shutdown_);
}