...
std::vector<int16_t> ExtractPrimaryChannel(const std::vector<int16_t>& data, int channels) {
    if (channels <= 1) {
        return data;
    }

    std::vector<int16_t> mono_data(data.size() / channels);
    for (size_t frame = 0; frame < mono_data.size(); ++frame) {
        mono_data[frame] = data[frame * channels];
    }
    return mono_data;
}
...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    ...
    if (codec_->input_sample_rate() != sample_rate) {
        ...
        data.resize(samples * codec_->input_sample_rate() / sample_rate * input_channels);
        if (!codec_->InputData(data)) {
            return false;
        }
        if (input_channels > 1) {
            size_t input_frames = data.size() / input_channels;
            size_t output_frames = input_resamplers_[0].GetOutputSamples(input_frames);
            std::vector<std::vector<int16_t>> separated_channels(input_channels, std::vector<int16_t>(input_frames));
            for (size_t frame = 0; frame < input_frames; ++frame) {
                for (int channel = 0; channel < input_channels; ++channel) {
                    separated_channels[channel][frame] = data[frame * input_channels + channel];
                }
            }

            std::vector<std::vector<int16_t>> resampled_channels(input_channels, std::vector<int16_t>(output_frames));
            for (int channel = 0; channel < input_channels; ++channel) {
                input_resamplers_[channel].Process(
                    separated_channels[channel].data(),
                    separated_channels[channel].size(),
                    resampled_channels[channel].data());
            }

            data.resize(output_frames * input_channels);
            for (size_t frame = 0; frame < output_frames; ++frame) {
                for (int channel = 0; channel < input_channels; ++channel) {
                    data[frame * input_channels + channel] = resampled_channels[channel][frame];
                }
            }
        } else {
            auto resampled = std::vector<int16_t>(input_resamplers_[0].GetOutputSamples(data.size()));
        ...
        if (bits & AS_EVENT_AUDIO_TESTING_RUNNING) {
            ...
            if (ReadAudioData(data, 16000, samples)) {
                if (codec_->input_channels() > 1) {
                    data = ExtractPrimaryChannel(data, codec_->input_channels());
                }
